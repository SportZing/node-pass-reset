...
exports.requestResetToken = function(opts) {
	...
	var func = function(req, res, next) {
		...
		if (! login) {
			return res.json(json.error('No login given'), 400);
		}
		lookupUsers(login, function(err, users) {
			if (err) {
				return res.json(json.error(err), 500);
			}
			if (! users) {
				return res.json(json.error('No such user'), 404);
			}
...
exports.resetPassword = function(opts) {
	...
	var func = function(req, res, next) {
		...
		if (! params.token || ! params.password || ! params.confirm) {
			return res.json(json.error('Cannot attempt reset with missing params'), 400);
		}
		var id = storage.lookup(params.token);
		if (! id) {
			return res.json(json.error('Request token is invalid'), 401);
		}
		if (params.password !== params.confirm) {
			return res.json(json.error('Password and confirmation do not match'), 400);
		}
		setPassword(id, params.password,
			function(err, success, validationError) {
				...
				if (! success) {
					return res.json(json.error(validationError), 400);
				}
